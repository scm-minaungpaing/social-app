...
const AuthController = {
    register: async (req, res) => {
        try {
            ...
            if (user_name) return res.status(400).json({ msg: "This user name is already exits." })

            const user_email = await Users.findOne({ email })
            if (user_email) return res.status(400).json({ msg: "This email is already exits." })
            ...
            const access_token = createAceessToken({ id: newUser._id })
    ...
    generateAccessToken: async (req, res) => {
        try {
            ...
            jwt.verify(rf_token, process.env.REFRESH_TOKEN, async(err, result) => {
                ...
                const access_token = createAceessToken({id: result.id})
...
const createAceessToken = (payload) => {
